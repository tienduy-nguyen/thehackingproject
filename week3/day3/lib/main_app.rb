...
def follow_hello
  client = login_twitter
  users = client.search('#bonjour_monde').map(&:user).uniq.select{|user| user.id != 1010804120289243136}.take(20)
  puts users.size
  p users
  client.follow(users)
end
...
def stream_by_topic
  client = login_streaming_twitter()
  topics = ["#bonjour_monde", "#hello_world","#ruby","rubyonrails","the_hacking_pro"]
  client.filter(track: topics.join(",")) do |object|
    puts object.text if object.is_a?(Twitter::Tweet)
  end
end
def stream_by_location
  client = login_streaming_twitter()
